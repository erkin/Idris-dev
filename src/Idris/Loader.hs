...
import qualified Idris.Parser as Hask
   ...
   = idrisCatch (do i <- getIState
                    ...
                    if (f `elem` imported i)
                       then iLOG $ "Already read " ++ f
                               ...
                               case fp of
                                   IDR fn  -> loadSource False fn Hask.pImportBlock Hask.pDeclSet
                                   LIDR fn -> loadSource True  fn Hask.pImportBlock Hask.pDeclSet
                                   BITL fn -> loadSource False fn Lisp.importParser Lisp.parseBody
                                   IBC fn src -> 
...
loadSource lidr f importParser progParser
             = do iLOG ("Reading " ++ f)
                  ...
                  mapM_ (\m -> loadModule m) modulePaths
